...
import {
  ...
  DEFAULT_DENOMINATION_TYPES, DEFAULT_SECTIONS_DATA,
} from './models.js';
...
/**
 * Build a fresh CasinoData from the current settings.
 * Existing data is NOT preserved — use applySettingsToCasinoData for that.
 */
export function createDefaultCasinoData(settings) {
  ...
  for (const si of settings.sectionsData) {
    ...
    for (const tableName of si.tablesNames) {
      section.tables.push(buildTable(tableName, sortedDenom));
    }
...
/**
 * Rebuild CasinoData to match the new settings while preserving existing
 * table values where section / table names overlap.
 */
export function applySettingsToCasinoData(settings, casinoData) {
  ...
  for (const si of settings.sectionsData) {
    ...
    for (const tableName of si.tablesNames) {
      ...
      const table         = buildTable(tableName, sortedDenom, existingTable);
...
function buildTable(name, sortedDenom, existingTable = null) {
  const table = createTable(name);

  if (existingTable) {
    // Preserve identity & numeric fields
  ...
  for (const dt of sortedDenom) {
    const existing = existingTable?.denominationCounts.find(dc => dc.denominationType.name === dt.name);
